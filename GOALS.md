...
| 2. Choix des bobines affiché dans Mainsail / Fluidd | **écrit et testé, à déployer (PR point 2, sur PR #53)** | `kctrl_slot_map` apparie chaque filament déclaré par le fichier (type + couleur, exactement, la règle des groupes de relève du CFS) sur les bobines chargées et le publie (`match`) ; `START_PRINT` prend la bobine appariée avant la table et refuse avant de chauffer si le filament de départ n'a pas de bobine, en nommant la plus proche ; `KCTRL_MATCH` écrit les filaments utilisés dans la table (ADR-062, document 75). `MATCH=0` ou `TOOL=` gardent l'ancien comportement. Reste : déploiement, première observation sur un démarrage Mainsail dont la couleur ne colle pas à la table. |
