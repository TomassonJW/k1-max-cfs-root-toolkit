...
| 2. Choix des bobines affiché dans Mainsail / Fluidd | **refait selon la consigne du 12 septembre : choix obligatoire, à la main, avant chaque départ (ADR-063, document 78) ; installé le 12 septembre au soir, fenêtre dans Mainsail à 22:52** | Chaque départ (Mainsail, Fluidd, écran, Creality Print) est retenu par `kctrl_print_gate` avant toute chauffe ; dans Mainsail, la fenêtre Bobines couvre l'écran d'elle-même (0,4 s après le départ du cube, 2,4 s sur un fichier de 50 Mo) avec les filaments du fichier et toutes les bobines du CFS (couleur, matière, reste), Thomas raccorde d'un clic, « Lancer l'impression » n'est actif qu'une fois tout raccordé, et l'impression part ; « Réduire » laisse une pastille. La même interface en page seule (`http://192.168.1.64:4409/bobines/`) pour un téléphone, Fluidd ou l'écran. La confirmation écrit la table CFS et `START_PRINT` la prend telle quelle (« raccordé sur la page Bobines »). L'appariement automatique (ADR-062, document 75) reste en repli (reprise après coupure, `MATCH=1`). Premier départ réel par la fenêtre le 12 septembre à 23:17 (BIN4U, « c'est ce que j'attendais »). Service de passerelle qui repose la balise à chaque démarrage posé le 14 septembre à 17:59 (« balise deja en place »). Envoi de G-code par le gestionnaire de fichiers de Mainsail réparé le même jour (refus de tout fichier de plus de 16 Ko, plafond 100 Mo → 1 Go) : 156 Mo envoyés en 59 s par le port 4409. |
...
| 4. Multi-filament en cours d'impression | **installé le 12 septembre ; impression multicouleur sans pause le 14 septembre de 22:30 à 22:54 (cinq filaments, quatre changements), ADR-065 validé ; reste lent : 3 min 22 par changement, dont 2 min 14 de purge (document 79, section 9)** | `kctrl_tool_change.py` reprend `T0`..`T15` : fiche du matériau visé alignée sur la température du fichier (première couche ou courante) avant la commande stock, refus net avant elle si l'emplacement est vide, l'unité absente ou le filament non déclaré, pause si la tête est vide après le changement, cible remise à la valeur du fichier (ADR-061, document 74). `KCTRL_TOOLS` dit ce que fera chaque filament. Observé sur `3DBenchy_C2` (quatre filaments) le 14 septembre : « T3 fait, filament a la tete » à 19:22:36, « T0 fait, cible 220 C » à 20:37:30. Mais le retrait de l'ancien filament vidait le capteur de tête que `START_PRINT` arme pour la relève : pause dès la fin du changement (19:22:36). ADR-065 coupe cette alarme pendant chaque changement et la rallume après ; le capteur reste lu. Impression finie à 20:56:58, T0 et T1 passés sans pause après la reprise. Impression `MultiColo_Cube_PLA_15m31s` suivie en direct le 14 septembre de 22:30:56 à 22:54:21 : cinq `T` avec l'alarme coupée puis rallumée, aucune pause, aucun code d'erreur du CFS. Reste : purge arrondie par le module Creality à 140 ou 280 mm et poussée à 140 mm/min (1120 mm pour 883 demandés), volumes du trancheur à aligner (document 79, section 10, correctif 1) et source de la vitesse à trouver (correctif 2) ; relance Creality « l'extrudeur n'a pas mordu » sur le noir T1A (2 chargements sur 2 le 14, 80 s perdues au départ, buse chaude), trois gestes pour trancher bobine ou emplacement (section 9.4) ; cutter intermittent (matériel) ; accumulateur Z rendu à l'identique sur les démarrages du 10 au soir. Départ : arrêt net, chauffes coupées, si le CFS abandonne ou si l'on met en pause, et deux tentatives de chargement au lieu de quatre (ADR-066, installé le 14 septembre à 22:16 ; départ de 22:30 passé sans arrêt, arrêt sur une vraie pause pas encore observé). Fin d'impression : la tête est vidée par nos soins avant la fin stock, coupe puis rembobinage de l'emplacement nommé (ADR-067, posé le 15 septembre à 14:03, validé sur la fin réelle de 18:28 : retrait de `T2B` en 28 s, `BOX_END` devant une tête vide en 14 s, 0 tronçon, document 82). Relances de chargement : 3 sur 5 le 15 septembre (`T1A`, `T2D`, `T1D`), 5 sur 6 depuis le CFS 1 sur deux impressions contre 1 sur 7 depuis le CFS 2 ; un chargement raté coûte 85 à 90 s ; geste pour trancher : échanger noir et bleu entre les deux CFS (document 82, section 8). Reprise après changement raté : le 23 septembre, RESUME rechargeait puis figeait l'impression (alarme de tête armée pendant le rechargement `T1A`) ; alarme coupée pendant les seize rechargements `T1A`..`T4D`, posé au repos le 24 septembre à 00:48, pas encore observé sur un vrai raté (ADR-072, document 99). |
| 5. Relève automatique en fin de bobine | **prouvée une fois, pas de paire aujourd'hui** | Fonction stock (`auto_refill: 1`) : capteur de tête → `runout_gcode` → `BOX_CHECK_MATERIAL_REFILL` → `BOX_MODIFY_TN` → `T` rejoué ; a fonctionné le 5 septembre à 15:22 (T1A → T1B, huit secondes). Ne relève qu'entre bobines de même fiche **et** même couleur (`same_material`) ; ce soir chaque bobine est seule dans son groupe, une fin de bobine finirait en pause. Procédure pour la provoquer (deux bobines identiques, impression sans valeur) : document 76. Rien à déployer. |
| 6. Zéro intervention | **pas encore** | Somme des points 2, 4 et 5. Calibrage : le maillage `11 × 11` du 10 septembre est faux, le firmware incline chaque mesure de 0,10 mm du premier rang au dernier (document 73) ; profil reconstruit depuis les contacts bruts, **appliqué le 10 septembre à 12:33** ; zéro Z à refaire sur le carré 280×280 (le +0,180 réglé sur le cube a été remis à zéro par l'interface à 11:43). Démarrage : une purge, une ligne (ADR-060), **déployé à 12:31, observé sur deux démarrages le soir** (une purge, une ligne, Z rendu). Z 0,065 enregistré par Thomas à 20:15. Bandes de température (ADR-064, installé le 14 septembre à 17:58, copie ramenée de 70 à 65 °C à 18:10) : un maillage sert à ±5 °C, le 55 couvre 50–60 °C ; `k1_p001_t065_r001_n11x11` copié du 55 couvre 61–70 °C, Z 0,065 recopié à affiner sur le carré ; aucun trou de 50 à 70 °C. Pauses parasites : trois `key831` le 15 septembre (05:15, 08:01, 09:24), hors de notre code ; le CFS 2 n'entend pas une question partie moins de 100 ms après la réponse du capteur du CFS 1 finie par `F7` (valeur 1, en veille depuis une tension de `T1A` à 03:45:58). Garde de 300 ms posée le 15 septembre à 17:52 (ADR-068) ; première impression sous la garde à 18:02 : 1 218 questions au CFS 2, 0 muette, 0 `key831`, une question retenue et répondue ; le cas du matin (rapport du capteur en veille toutes les 5 s) ne s'est pas présenté, preuve décisive encore à venir (document 82). Fin d'impression du 15 septembre : le module CFS a poussé environ 2 m de `T1A` en boucle au lieu de couper, et la Pause est restée sans effet. Klipper a planté à 12:20 à cause de notre lecture du journal. `T1A` rembobiné par Thomas à 13:23 (`BOX_RETRUDE_MATERIAL_WITH_TNN TNN=T1A`, 20 s). Document 81 écrit ; correctifs posés à 14:03 : la fin d'impression vide la tête elle-même avant la fin stock (ADR-067), les lectures du journal sont bornées et testées, l'audit en direct alerte sur la boucle de fin, la Pause sans effet, le journal muet et la mémoire basse. Garde du bus posée à 17:52 (ADR-068). Impression de 18:02 auditée (document 82) : fin propre en 42 s, zéro pause, zéro silence. |
