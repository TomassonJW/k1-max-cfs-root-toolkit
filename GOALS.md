...
| 4. Multi-filament en cours d'impression | **installé le 12 septembre ; première impression multicouleur le 14 septembre : chaque changement mettait en pause ; correctif ADR-065 installé le 14 septembre à 21:00, à observer** | `kctrl_tool_change.py` reprend `T0`..`T15` : fiche du matériau visé alignée sur la température du fichier (première couche ou courante) avant la commande stock, refus net avant elle si l'emplacement est vide, l'unité absente ou le filament non déclaré, pause si la tête est vide après le changement, cible remise à la valeur du fichier (ADR-061, document 74). `KCTRL_TOOLS` dit ce que fera chaque filament. Observé sur `3DBenchy_C2` (quatre filaments) le 14 septembre : « T3 fait, filament a la tete » à 19:22:36, « T0 fait, cible 220 C » à 20:37:30. Mais le retrait de l'ancien filament vidait le capteur de tête que `START_PRINT` arme pour la relève : pause dès la fin du changement (19:22:36). ADR-065 coupe cette alarme pendant chaque changement et la rallume après ; le capteur reste lu. Impression finie à 20:56:58, T0 et T1 passés sans pause après la reprise. Reste : une impression multicouleur sans pause avec ADR-065 ; cutter intermittent (matériel) ; accumulateur Z rendu à l'identique sur les démarrages du 10 au soir ; relance Creality « l'extrudeur n'a pas mordu » (coupe, rembobinage, réinsertion) sur 2 changements sur 3 le 14 septembre, cause à trouver pendant l'audit en direct (document 79). Départ : arrêt net, chauffes coupées, si le CFS abandonne ou si l'on met en pause, et deux tentatives de chargement au lieu de quatre (ADR-066, installé le 14 septembre à 22:16, à observer sur l'impression suivie en direct). |
