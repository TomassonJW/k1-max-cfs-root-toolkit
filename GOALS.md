...
| 4. Multi-filament en cours d'impression | **écrit et testé, à déployer (PR #53)** | `kctrl_tool_change.py` reprend `T0`..`T15` : fiche du matériau visé alignée sur la température du fichier (première couche ou courante) avant la commande stock, refus net avant elle si l'emplacement est vide, l'unité absente ou le filament non déclaré, pause si la tête est vide après le changement, cible remise à la valeur du fichier (ADR-061, document 74). `KCTRL_TOOLS` dit ce que fera chaque filament. Reste : déploiement entre deux impressions, première observation sur un fichier à deux filaments ; cutter intermittent (matériel) ; accumulateur Z rendu à l'identique sur les démarrages du 10 au soir. |
