...
| 2. Choix des bobines affiché dans Mainsail / Fluidd | **refait selon la consigne du 12 septembre : choix obligatoire, à la main, avant chaque départ (ADR-063, document 78) ; installé le 12 septembre au soir** | Chaque départ (Mainsail, Fluidd, écran, Creality Print) est retenu par `kctrl_print_gate` avant toute chauffe ; la page Bobines (`http://192.168.1.64:4409/bobines/`) montre les filaments du fichier et toutes les bobines du CFS (couleur, matière, reste), Thomas raccorde d'un clic, le bouton de lancement n'apparaît qu'une fois tout raccordé ; la confirmation écrit la table CFS et `START_PRINT` la prend telle quelle (« raccordé sur la page Bobines »). L'appariement automatique (ADR-062, document 75) reste en repli (reprise après coupure, `MATCH=1`). Reste : premier départ réel par la page, à observer par Thomas. |
