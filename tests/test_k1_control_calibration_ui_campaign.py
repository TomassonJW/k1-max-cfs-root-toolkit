...
class CalibrationUiCampaignContractTests(unittest.TestCase):
    def test_execution_manifest_pins_every_reviewed_artifact(self):
        ...
        self.assertEqual(manifest["mesh_measurements_per_level"], 1)
        self.assertEqual(manifest["physical_levels"], 1)
        self.assertEqual(manifest["total_mesh_measurements"], 1)
    ...
    def test_campaign_is_separate_gated_and_requires_browser_only_control(self):
        ...
        self.assertEqual(
            ...
            "deployed_validated_and_rendered_in_browser_with_fixed_6x6_grid",
        )
        self.assertFalse(
    ...
    def test_all_matrix_levels_require_real_physical_mesh_proof(self):
        ...
        self.assertEqual(capability["physical_proof"], "one_complete_36_point_mesh")
        self.assertEqual(capability["full_z_workflow_preset"], "supported")
    ...
    def test_exact_proven_settings_and_final_guards_are_declared(self):
        ...
        self.assertEqual([item["name"] for item in sequence], ["supported"])
        self.assertEqual([item["matrix"] for item in sequence], [[6, 6]])
        self.assertEqual([item["interpolation"] for item in sequence], ["lagrange"])
        self.assertEqual([item["replace_existing"] for item in sequence], [True])
        results = contract["required_results"]
        self.assertEqual(results["mesh_measurements"], 1)
        self.assertEqual(results["physical_points"], 36)
    ...
    def test_runbook_rejects_console_assistance_and_reruns(self):
        ...
        self.assertIn("Aucun deuxième passage", readme)
        self.assertIn("Aucun rerun automatique", readme)
        self.assertIn("un seul mesh complet", readme)
    ...
    def test_validator_is_read_only_and_checks_the_full_final_state(self):
        ...
        self.assertIn("@($privateState.meshes).Count -ne 1", source)
        self.assertIn("Assert-SafeAcceptedMachine", source)
        self.assertIn("VALIDATE_CALIBRATION_UI_CAMPAIGN_V1_OK", source)
        self.assertIn("ExpectedBedMeshManifestHash", source)
        self.assertIn("BedMeshManifest", source)
        self.assertIn("ExpectedPresetManifestHash", source)
        self.assertIn("PresetManifest", source)
        self.assertIn("$requiredDestinations", source)
        self.assertIn("@($PresetManifest.files) + @($PresetManifest.unchanged.files)", source)
        self.assertIn("printer.cfg distant différent de la base revue", source)
        self.assertIn("Assert-CampaignPrinterConfig", source)
        self.assertIn("$Action -in @('CaptureLevel', 'Validate')", source)
        self.assertIn("printer.cfg a changé hors des points du profil qualifié", source)
        self.assertIn("Valeur mesh persistante différente de la mesure acceptée", source)
        self.assertIn("[string]$PrivateState.backup.printer_cfg_sha256 -cne $expectedHash", source)
        self.assertIn("Le manifeste final PRESETS ne couvre pas la destination revue", source)
        self.assertIn("Payload final PRESETS distant inattendu", source)
        self.assertIn("failed_components", source)
        self.assertIn("warnings", source)
        self.assertIn("configfile.settings.bed_mesh.probe_count", source)
        self.assertIn("$unit.version -cne '1.1.3'", source)
