...
SPEC = importlib.util.spec_from_file_location('overlap_end', ROOT / 'packages/k1-control-v1/end-overlap-v2/kctrl_end.py')
...
def test_delayed_route_clear_waits_at_cutter_without_z_or_bin_move():
    ...
    expected = ['BOX_CUT_MATERIAL', 'LOCAL_CUTTER_RELEASE', 'M83', 'G1 E-20 F5000', 'M400', 'G1 E-15 F120',
                'BOX_SET_BOX_MODE ADDR=1 MODE=IDLE NUM=0', 'BOX_CTRL_CONNECTION_MOTOR_ACTION ADDR=1 ACTION=STOP', REWIND, 'M400', 'END_PRINT_NO_M84']
...
@pytest.mark.parametrize('ack_seconds', [.05, 2., 12.])
@pytest.mark.parametrize('candidate', ['overlap_v2', 'broken_v1'])
def test_extruder_and_cfs_overlap_with_realistic_motion_queue(ack_seconds, candidate, monkeypatch):
    # Unlike the old string-order test, a G1 schedules motion and M400 drains
    # it. Model a CFS that succeeds only when slow extrusion overlaps its start.
    if candidate == 'broken_v1':
        import test_kctrl_end_separated_v1 as old
        monkeypatch.setitem(globals(), 'END', old.END)
    p, c = setup()
    original = p.gcode.run_script_from_command
    queue = {'end': 0., 'moves': [], 'cfs_at': None, 'park_at': None}
    def script(text):
        if text.startswith('G1 E'):
            fields = text.split()
            distance, speed = float(fields[1][1:]), float(fields[2][1:])
            start = max(p.reactor.now, queue['end'])
            end = start + abs(distance) / (speed / 60.)
            queue['moves'].append((distance, start, end))
            queue['end'] = end
        elif text == 'M400':
            p.reactor.pause(max(p.reactor.now, queue['end']))
        elif text == 'END_PRINT_NO_M84':
            queue['park_at'] = p.reactor.now
            assert p.reactor.now >= queue['end']
        original(text)
    p.gcode.run_script_from_command = script
    def cfs():
        fast, slow = queue['moves']
        queue['cfs_at'] = p.reactor.now
        assert fast[2] <= slow[1] <= p.reactor.now < slow[2]
        p.reactor.pause(p.reactor.now + ack_seconds)
        p.obj['box'].status['T1']['filament'] = 'None'
        p.obj['filament_switch_sensor filament_sensor_2'].status['filament_detected'] = False
    p.gcode.on_script[REWIND] = cfs
    finish(p, c)
    if candidate == 'broken_v1':
        base.assert_safe_failure(p, c)
        assert queue['cfs_at'] >= queue['moves'][-1][2]
        assert queue['park_at'] is None
        return
    assert c.phase == 'complete'
    assert [m[0] for m in queue['moves']] == [-20., -15.]
    assert queue['park_at'] >= queue['moves'][-1][2]
    assert queue['cfs_at'] < queue['moves'][-1][2]
    assert p.gcode.scripts.count(REWIND) == 1


def test_real_key849_response_keeps_failure_and_heaters_off():
    p, c = setup()
    def cfs():
        p.gcode.emit('!! {"code":"key849", "msg":"retrude error, failed to exit connections", "values":[1,"B"]}')
    p.gcode.on_script[REWIND] = cfs
    finish(p, c)
    base.assert_safe_failure(p, c)
    assert c.failure == 'firmware_reported_error'
    assert p.obj['filament_switch_sensor filament_sensor_2'].status['filament_detected']
    assert p.gcode.scripts.count(REWIND) == 1
