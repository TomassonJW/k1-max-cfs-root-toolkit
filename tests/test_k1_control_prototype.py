...
class StaticInterfaceTests(unittest.TestCase):
    ...
    def test_real_adapter_is_fail_closed_and_has_no_arbitrary_gcode_path(self) -> None:
        ...
        self.assertIn('const RUNTIME_OBJECT = "gcode_macro KCTRL_STATE"', adapter)
        self.assertIn("Runtime K1 Control non installé", adapter)
        self.assertIn("EXACT_COMMANDS", adapter)
        self.assertIn("COMMAND_PATTERNS", adapter)
        self.assertIn("liste blanche K1 Control", adapter)
        self.assertIn("KCTRL_MESH_CALIBRATE", adapter)
        self.assertIn("KCTRL_CALIBRATION_PREHEAT", adapter)
    ...
    def test_simulated_moonraker_applies_the_python_z_state_rules(self) -> None:
        ...
        simulation.dispatch_script(f"KCTRL_Z_SESSION_START SEED={before}")
        simulation.dispatch_script("KCTRL_Z_ADJUST DELTA=0.005")
        ...
        simulation.dispatch_script("KCTRL_Z_COMMIT")
