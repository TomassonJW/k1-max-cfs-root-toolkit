...
class OrcaControlContractTests(unittest.TestCase):
    ...
    def test_start_has_no_direct_homing_tool_or_temperature_command(self) -> None:
        ...
        self.assertTrue(executable[-1].endswith("K1_JOB_START CONTRACT=1"))
        ...
        self.assertEqual(start.count("K1_JOB_TOOL_TARGET"), 8)
    ...
    def test_tool_change_wraps_the_stock_cfs_boundary_with_dynamic_targets(self) -> None:
        ...
        self.assertLess(change.index("K1_TOOL_CHANGE_BEGIN"), change.index("T{next_extruder}"))
        self.assertLess(change.index("T{next_extruder}"), change.index("K1_TOOL_CHANGE_END"))
    ...
    def test_end_never_commits_or_resets_z(self) -> None:
        ...
        self.assertIn("K1_JOB_END CONTRACT=1", end)
