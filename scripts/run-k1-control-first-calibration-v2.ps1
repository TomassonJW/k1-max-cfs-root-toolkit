[CmdletBinding()]
param(
    [ValidateSet('Plan', 'Preflight', 'Prepare', 'Mesh1', 'Mesh2', 'Mesh3', 'Mesh4', 'Mesh5', 'Mesh6AndQualify', 'CommitRobustMesh', 'BeginZ', 'StepZ', 'AdjustZ', 'ConfirmGap', 'Accept', 'Cancel', 'Validate', 'Rollback')]
    [string]$Action = 'Plan',

    [string]$Gate,

    [ValidatePattern('^[0-9]{8}-[0-9]{6}-g4-k1-control-first-calibration-v2$')]
    [string]$CaptureId,
...
$RequiredGate = 'G4-K1-CONTROL-FIRST-CALIBRATION-V2'
$WorkspaceRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$PackageRoot = Join-Path $WorkspaceRoot 'packages\k1-control-v1\first-calibration-v2'
$ContractPath = Join-Path $PackageRoot 'first-calibration-contract.json'
$AggregateScript = Join-Path $PackageRoot 'aggregate_meshes.py'

$ExpectedContractHash = '7272e47777d22bd075464e7843e703f7431ba97d302825186efeec06e3417e7d'
$ExpectedAggregateHash = '7668bdcb321748289fbcb35bf053f378ef51f6cf13ec05f451c660fd0e9b9031'
...
function Assert-ReviewedLocalFiles {
    foreach ($item in @(
            ...
            @{ Path = $AggregateScript; Expected = $ExpectedAggregateHash; Label = 'agregateur' }
        )) {
...
function Test-ReviewedGcode {
    ...
    $fixed = @(
        ...
        'BED_MESH_CLEAR',
        'BED_MESH_PROFILE LOAD=K1_TRANSIENT',
...
function Invoke-KlipperMeshUpdate {
    param([Parameter(Mandatory = $true)]$Matrix)
    $matrixJson = $Matrix | ConvertTo-Json -Depth 5 -Compress
    $matrixPayload = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($matrixJson))
    $python = @'
from __future__ import print_function
import base64
import json
...
matrix = json.loads(base64.b64decode(sys.argv[1]).decode("utf-8"))
request = {"id": 5403, "method": "update_mesh", "params": {"probed_matrix": matrix}}
client = socket.socket(socket.AF_UNIX, socket.SOCK_STREAM)
...
time.sleep(0.2)
client.close()
print(json.dumps({"sent": "update_mesh", "rows": len(matrix)}))
'@
    $payload = [Convert]::ToBase64String([Text.Encoding]::UTF8.GetBytes($python.Replace("`r`n", "`n")))
    $line = Invoke-Remote "echo '$payload' | base64 -d | /usr/share/klippy-env/bin/python - '$matrixPayload'"
    return (($line -join "`n") | ConvertFrom-Json)
}
...
function Invoke-FreshPreflight {
    ...
    if (-not (Get-Command python.exe -ErrorAction SilentlyContinue)) {
        throw 'Python local absent pour la qualification des six meshes.'
    }
    ...
    if ($snapshot.toolhead.homed_axes -and $snapshot.toolhead.homed_axes -ne 'xyz') {
        throw "Etat de homing partiel inattendu : $($snapshot.toolhead.homed_axes)"
    }
...
function Get-RemoteBackupRoot {
    ...
    return "$RemoteRoot/backups/$CaptureId/first-calibration-v2"
}
...
function Assert-QualifiedEvidence {
    ...
    if (-not $qualification.accepted -or [int]$qualification.measurements -ne 6 -or
        [int]$qualification.batch_size -ne 3 -or [int]$qualification.compared_points -ne 36 -or
        [double]$qualification.observed_mm.mean_absolute -gt 0.020 -or
        [double]$qualification.observed_mm.rms -gt 0.025 -or
        [double]$qualification.observed_mm.maximum -gt 0.060 -or
        @($qualification.candidate_matrix).Count -ne 6) {
        throw 'La qualification robuste locale des six meshes n est pas acceptee.'
    }
    return $qualification
}

function Assert-CandidateMatrix {
    param(
        [Parameter(Mandatory = $true)]$Actual,
        [Parameter(Mandatory = $true)]$Expected
    )
    if (@($Actual).Count -ne 6 -or @($Expected).Count -ne 6) {
        throw 'Matrice candidate ou relue de dimension invalide.'
    }
    for ($row = 0; $row -lt 6; $row++) {
        if (@($Actual[$row]).Count -ne 6 -or @($Expected[$row]).Count -ne 6) {
            throw 'Ligne de matrice candidate ou relue de dimension invalide.'
        }
        for ($column = 0; $column -lt 6; $column++) {
            if ([math]::Abs([double]$Actual[$row][$column] - [double]$Expected[$row][$column]) -gt 0.000001) {
                throw "Matrice robuste differente apres relecture au point [$row,$column]."
            }
        }
    }
}
...
if ($Action -eq 'Plan') {
    ...
    [pscustomobject]@{
        status = 'PLAN_ONLY'
        gate = $RequiredGate
        printer_mutation_authorized = $true
        plate = 'PEI_TEXTURED_A (id=1)'
        temperatures_c = @{ bed = 55; nozzle = 140; cleaning_max = 180 }
        soak_seconds = 200
        mesh = 'six fixed 6x6 Lagrange measurements over 5-295 mm; two independent medians of three'
        qualification_mm = @{ mean_absolute = 0.020; rms = 0.025; maximum = 0.060 }
        automatic_extra_measurement = $false
...
if ($Action -eq 'Preflight') {
    ...
    Write-Output 'PREFLIGHT_FIRST_CALIBRATION_V2_OK'
    exit 0
}
...
if ($Action -eq 'Prepare') {
    ...
    try {
        Invoke-KlipperScript 'BED_MESH_CLEAR' | Out-Null
        ...
        Write-Output "PREPARE_FIRST_CALIBRATION_V2_OK capture=$CaptureId"
    }
    catch {
...
if ($meshActions.ContainsKey($Action)) {
    $meshNumber = [int]$meshActions[$Action]
    if ($meshNumber -eq 1) {
        [void](Assert-Checkpoint 'prepare.json')
    }
    else {
        [void](Assert-Checkpoint ("mesh-{0}.json" -f ($meshNumber - 1)))
    }
    [void](Assert-RemoteBackup)
    try {
        $before = Get-KlipperSnapshot
        if ($meshNumber -eq 1) { Assert-PreparedSnapshot $before } else { Assert-TransientMesh $before }
        Invoke-KlipperScript 'KCTRL_MESH_CALIBRATE X_COUNT=6 Y_COUNT=6 ALGORITHM=lagrange' | Out-Null
        $snapshot = Get-KlipperSnapshot
        Assert-TransientMesh $snapshot
        Save-Evidence ("mesh-{0}.json" -f $meshNumber) $snapshot
        if ($meshNumber -eq 6) {
            $meshPaths = 1..6 | ForEach-Object { Assert-Checkpoint ("mesh-{0}.json" -f $_) }
            $qualificationOutput = & python.exe $AggregateScript @meshPaths 2>&1
            $qualificationCode = $LASTEXITCODE
            if ($qualificationCode -notin @(0, 2)) {
                throw "Agregateur mesh KO ($qualificationCode) : $qualificationOutput"
            }
            Save-Evidence 'mesh-qualification.json' ($qualificationOutput -join "`n")
            if ($qualificationCode -eq 2) {
                Invoke-TurnOffHeatersBestEffort
                throw 'MESH6_FIRST_CALIBRATION_V2_KO : les deux medianes independantes divergent ; aucun septieme mesh.'
            }
            [void](Assert-QualifiedEvidence)
        }
        Write-Output "MESH${meshNumber}_FIRST_CALIBRATION_V2_OK capture=$CaptureId"
    }
    catch {
        Invoke-TurnOffHeatersBestEffort
        throw
...
if ($Action -eq 'CommitRobustMesh') {
    $qualification = Assert-QualifiedEvidence
    [void](Assert-RemoteBackup)
    [void](Assert-Checkpoint 'mesh-6.json')
    $snapshot = Get-KlipperSnapshot
    Assert-TransientMesh $snapshot
    try {
        ...
        Assert-CandidateMatrix -Actual $loaded.bed_mesh.probed_matrix -Expected $qualification.candidate_matrix
        Save-Evidence 'mesh-robust-loaded.json' $loaded
        ...
        if ($after.bed_mesh.profiles.PSObject.Properties.Name -notcontains $MeshProfile -or
            $after.bed_mesh.profiles.PSObject.Properties.Name -contains 'K1_TRANSIENT') {
            throw 'Profil robuste final absent ou profil transitoire encore present.'
        }
        $savedMatrix = $after.bed_mesh.profiles.$MeshProfile.points
        Assert-CandidateMatrix -Actual $savedMatrix -Expected $qualification.candidate_matrix
        Save-Evidence 'mesh-commit.json' $after
        Save-Evidence 'mesh-commit-printer-sha256.txt' (Get-RemoteHash $PrinterConfig)
        Write-Output "COMMIT_ROBUST_MESH_FIRST_CALIBRATION_V2_OK capture=$CaptureId"
    }
    catch {
        Invoke-TurnOffHeatersBestEffort
        throw
...
if ($Action -eq 'BeginZ') {
    ...
    try {
        ...
        Write-Output "BEGIN_Z_FIRST_CALIBRATION_V2_OK capture=$CaptureId"
    }
    catch {
        Invoke-TurnOffHeatersBestEffort
        throw
...
if ($Action -eq 'StepZ') {
    ...
    Write-Output "STEP_Z_FIRST_CALIBRATION_V2_OK height=$Height capture=$CaptureId"
    exit 0
}
...
if ($Action -eq 'AdjustZ') {
    ...
    Write-Output "ADJUST_Z_FIRST_CALIBRATION_V2_OK delta=$Delta capture=$CaptureId"
    exit 0
}
...
if ($Action -eq 'ConfirmGap') {
    ...
    Write-Output "CONFIRM_GAP_FIRST_CALIBRATION_V2_OK capture=$CaptureId"
    exit 0
}
...
if ($Action -eq 'Accept') {
    ...
    Write-Output "ACCEPT_FIRST_CALIBRATION_V2_OK accepted_at=$acceptedAt capture=$CaptureId"
    exit 0
}
...
if ($Action -eq 'Cancel') {
    ...
    Write-Output "CANCEL_FIRST_CALIBRATION_V2_OK capture=$CaptureId"
    exit 0
}
...
if ($Action -eq 'Validate') {
    ...
    Write-Output 'VALIDATE_FIRST_CALIBRATION_V2_OK'
    exit 0
}
...
if ($Action -eq 'Rollback') {
    ...
    Write-Output "ROLLBACK_FIRST_CALIBRATION_V2_OK capture=$CaptureId"
    exit 0
}
