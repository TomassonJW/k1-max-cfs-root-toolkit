...
param(
    ...
    [ValidatePattern('^[0-9]{8}-[0-9]{6}-g4-k1-control-start-sequence-owner-preinsert-geometry-r4$')]
    [string]$CaptureId,
...
$RequiredGate = 'G4-K1-CONTROL-START-SEQUENCE-OWNER-PREINSERT-GEOMETRY-R4'
$WorkspaceRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$PackageRoot = Join-Path $WorkspaceRoot 'packages\k1-control-v1\start-sequence-owner-preinsert-geometry-r4'
$V1PackageRoot = Join-Path $WorkspaceRoot 'packages\k1-control-v1\start-sequence-owner-v1'
$LocalConfig = Join-Path $PackageRoot 'k1-control-start-sequence-owner-preinsert-geometry-r4.cfg'
...
function Assert-SafeSnapshot {
    ...
    $homedAxes = [string]$Snapshot.toolhead.homed_axes
    if ($homedAxes -ne '') {
        $position = @($Snapshot.toolhead.position)
        if ($homedAxes -ne 'xyz' -or $position.Count -lt 3 -or
            [double]$position[0] -lt 200.0 -or [double]$position[0] -gt 220.0 -or
            [double]$position[1] -lt 270.0 -or [double]$position[1] -gt 300.0 -or
            [double]$position[2] -lt 50.0 -or [double]$position[2] -gt 315.0) {
            throw 'Les axes ne sont ni liberes ni dans le parc haut qualifie.'
        }
    }
    ...
    if (-not $owner -or [int]$owner.manual_clean_token -ne 0 -or [int]$owner.geometry_ready_token -ne 0 -or
        [int]$owner.watchdog_armed -ne 0 -or
        [double]$owner.manual_clean_deadline -ne 0.0 -or [double]$owner.watchdog_deadline -ne 0.0 -or
        [double]$owner.geometry_ready_deadline -ne 0.0 -or
        $owner.phase -notin @('idle', 'watchdog_aborted')) {
        throw 'Etat du proprietaire non ferme.'
    }
    if ([double]$Snapshot.bounds.stepper_x.position_min -gt -1.7 -or
        [double]$Snapshot.bounds.stepper_x.position_max -lt 206.0 -or
        [double]$Snapshot.bounds.stepper_y.position_min -gt 20.0 -or
        [double]$Snapshot.bounds.stepper_y.position_max -lt 305.0 -or
        [double]$Snapshot.bounds.stepper_z.position_min -gt 0.3 -or
        [double]$Snapshot.bounds.stepper_z.position_max -lt 32.0) {
        throw 'Une trajectoire R4 sort des courses observees.'
    }
}
...
function Invoke-Preflight {
    ...
    return [pscustomobject]@{
        status = 'PREFLIGHT_START_SEQUENCE_OWNER_PREINSERT_GEOMETRY_R4_OK'
...
function Invoke-Rollback {
    ...
    $backup = "$RemoteRoot/backups/$CaptureId/start-sequence-owner-preinsert-geometry-r4"
    try {
        foreach ($command in @(
...
if ($Action -eq 'Deploy') {
    ...
    $backup = "$RemoteRoot/backups/$CaptureId/start-sequence-owner-preinsert-geometry-r4"
    $staging = "$RemoteRoot/staging/$CaptureId/start-sequence-owner-preinsert-geometry-r4"
    try {
        ...
        & scp.exe -O -q -o BatchMode=yes -o PasswordAuthentication=no -o KbdInteractiveAuthentication=no `
            $LocalConfig "k1max-root`:$staging/k1-control-start-sequence-owner-preinsert-geometry-r4.cfg"
        if ($LASTEXITCODE -ne 0) { throw 'Transfert du candidat R4 KO.' }
        if ((Get-RemoteHash "$staging/k1-control-start-sequence-owner-preinsert-geometry-r4.cfg") -ne $ExpectedNewConfigHash) {
            throw 'Candidat R4 distant different.'
        }
        ...
        Invoke-Remote "cp '$staging/k1-control-start-sequence-owner-preinsert-geometry-r4.cfg' '$StartOwnerConfig.next' && chmod 600 '$StartOwnerConfig.next' && mv '$StartOwnerConfig.next' '$StartOwnerConfig' && sync" | Out-Null
        Invoke-RestartAndRestoreMesh | Out-Null
        Invoke-ColdValidation | Out-Null
        Write-Output "DEPLOY_START_SEQUENCE_OWNER_PREINSERT_GEOMETRY_R4_OK capture=$CaptureId"
    }
    catch {
        $deploymentError = $_
        if ($MutationStarted) {
...
if ($Action -eq 'Validate') {
    Invoke-ColdValidation | Out-Null
    Write-Output 'VALIDATE_START_SEQUENCE_OWNER_PREINSERT_GEOMETRY_R4_OK'
    exit 0
}

if ($Action -eq 'Rollback') {
    Invoke-Rollback
    Write-Output "ROLLBACK_START_SEQUENCE_OWNER_PREINSERT_GEOMETRY_R4_OK capture=$CaptureId"
    exit 0
}
