...
param(
    ...
    [string]$CaptureId = (Get-Date -Format 'yyyyMMdd-HHmmss') + '-g4-k1-control-calibration-ui-prtouch-matrix-v1',
    [switch]$Execute,
    [string]$Gate = ''
)

$ErrorActionPreference = 'Stop'
$RequiredGate = 'G4-K1-CONTROL-CALIBRATION-UI-PRTOUCH-MATRIX-V1'
$WorkspaceRoot = Split-Path -Parent $PSScriptRoot
$PackageRoot = Join-Path $WorkspaceRoot 'packages\k1-control-v1\calibration-ui-prtouch-matrix-v1'
$ManifestPath = Join-Path $PackageRoot 'deployment-manifest.json'
$ContractPath = Join-Path $PackageRoot 'calibration-ui-prtouch-matrix-contract.json'
...
$RemoteBackup = "$RemoteRoot/backups/$CaptureId-calibration-ui-prtouch-matrix-v1"
$RemoteStaging = "$RemoteRoot/tmp/$CaptureId-calibration-ui-prtouch-matrix-v1"
...
function Assert-Package {
    ...
    if ($manifest.contract_id -cne $RequiredGate -or $manifest.status -cne 'offline_review_candidate') {
        throw 'Manifeste PRTOUCH-MATRIX-V1 inattendu.'
    }
    if ((Get-LocalSha256 $PSCommandPath) -cne [string]$manifest.deployer.sha256) {
        throw 'Empreinte du déployeur PRTOUCH-MATRIX-V1 inattendue.'
    }
    if ((Get-LocalSha256 $ContractPath) -cne [string]$manifest.contract.sha256) {
        throw 'Empreinte du contrat PRTOUCH-MATRIX-V1 inattendue.'
    }
...
document = b'[bed_mesh]\nprobe_count: 6,6\n[printer]\nkinematics: corexy\n'
rewritten, previous = module.ProbeCountFile._rewrite(document, (9, 9))
assert previous == (6, 6)
assert b'probe_count: 9,9' in rewritten
try:
    module.ProbeCountFile._rewrite(document, (4, 4))
except module.ProbeCountError:
    pass
else:
    raise AssertionError('4x4 must fail closed')
print('REMOTE_PRTOUCH_MATRIX_IMPORT_OK')
"@
    $arguments = @(
        '-o', 'BatchMode=yes',
        '-o', 'PasswordAuthentication=no',
    ...
    if ($LASTEXITCODE -ne 0 -or ($output | Select-Object -Last 1) -cne 'REMOTE_PRTOUCH_MATRIX_IMPORT_OK') {
        throw "Import Python distant PRTOUCH-MATRIX-V1 KO : $($output -join "`n")"
    }
}

function Assert-Baseline {
    param([Parameter(Mandatory = $true)]$Manifest)
    [void](Invoke-Remote "test -x '$MoonrakerService' && test -S /tmp/klippy_uds && test -f '$RemoteConfig' && test ! -e '$RemoteComponent'")
...
function Remove-RemoteStaging {
    [void](Invoke-Remote "rm -f '$RemoteStaging/k1_control_probe_count.py' '$RemoteStaging/moonraker.conf' && rmdir '$RemoteStaging' 2>/dev/null || true")
}
...
function Invoke-ExactRollback {
    ...
    [void](Invoke-Remote "cp '$RemoteBackup/moonraker.conf.before' '$RemoteConfig.rollback-next' && chmod 0644 '$RemoteConfig.rollback-next' && mv '$RemoteConfig.rollback-next' '$RemoteConfig'")
    [void](Invoke-Remote "rm -f '$RemoteComponent' '$RemoteComponents/__pycache__/k1_control_probe_count.cpython-38.pyc'")
    ...
    [void](Invoke-Remote "test ! -e '$RemoteComponent'")
    [void](Assert-SafeState)
}
...
if ($Action -eq 'Plan') {
    Write-Output "PLAN_CALIBRATION_UI_PRTOUCH_MATRIX_V1_OK gate=$RequiredGate"
    Write-Output 'Pose: ajout du composant, moonraker.conf dédié, restart Moonraker seulement.'
    Write-Output 'Exécution future: probe_count atomique après backup et avant chauffe, restart Klipper vérifié, restauration après chauffes.'
...
if ($Action -eq 'Preflight') {
    ...
    Write-Output 'PREFLIGHT_CALIBRATION_UI_PRTOUCH_MATRIX_V1_OK'
    exit 0
}

if ($Action -eq 'Validate') {
    Assert-Installed $manifest
    Write-Output 'VALIDATE_CALIBRATION_UI_PRTOUCH_MATRIX_V1_OK'
    exit 0
}
...
if ($Action -eq 'Rollback') {
    ...
    Write-Output "ROLLBACK_CALIBRATION_UI_PRTOUCH_MATRIX_V1_OK capture=$CaptureId"
    exit 0
}
...
try {
    ...
    [pscustomobject]@{
        ...
        gate = $RequiredGate
        result = 'DEPLOY_CALIBRATION_UI_PRTOUCH_MATRIX_V1_OK'
    ...
    Write-Output "DEPLOY_CALIBRATION_UI_PRTOUCH_MATRIX_V1_OK capture=$CaptureId"
}
catch {
    $failure = $_
    if ($MutationStarted) {
