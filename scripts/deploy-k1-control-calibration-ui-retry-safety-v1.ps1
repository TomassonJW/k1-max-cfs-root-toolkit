...
function Assert-SafeState {
    $api = Get-ApiState
    if ([bool]$api.busy -or @('idle', 'accepted', 'cancelled') -notcontains [string]$api.phase) {
...
if ($Action -eq 'Plan') {
    ...
    Write-Output 'Reprise incomplète: replacement et confirmation plateau remis à false une seule fois.'
...
if ($Action -eq 'Validate') {
    ...
    [void](Invoke-Remote "grep -q 'function isIncompleteRetry' '$RemoteApp' && grep -q 'plate-clear' '$RemoteApp'")
    [void](Assert-SafeState)
