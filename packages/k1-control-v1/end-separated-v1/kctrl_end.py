"""ADR-070 separated end: defer the end, resolve the live slot, prove the cut.
...
class KctrlEnd:
    ...
    def _separated_rewind(self, run):
        ...
        try:
            for command in ('M83', 'M221 S100', 'G1 E-20 F5000', 'G1 E-15 F120', 'M400'):
                self._unchanged_route(run)
    ...
    def get_status(self, eventtime=None):
        return {'enabled': self.enabled, 'phase': self.phase, 'reason': self.reason,
                ...
                'physical_validation': False, 'revision': 'separated-end-v1',
