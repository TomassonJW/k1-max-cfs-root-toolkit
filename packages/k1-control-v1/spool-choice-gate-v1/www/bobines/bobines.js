// The Bobines interface: what the printer publishes, drawn; what the operator
...
//
// One `mount(root, options)` draws it either on the stand-alone page
// (app.js, root = document) or in the window that overlay.js opens inside
// Mainsail (root = a shadow root, options.overlay = true). The interface is
// the same; only what surrounds it changes.
//
// It never decides anything. It shows the filaments the waiting file uses,
// the spools the CFS holds, and connects them one click at a time; the
// launch button only works once every used filament has a spool. The
// printer checks the same thing again in KCTRL_GATE_CONFIRM before starting.

import {
  NAMES, assign, buildModel, colourName, compatibility, completeness, elapsedLabel,
  isLight, mapParam, nextToConnect, normaliseHex, overlayMode, pendingKey,
  remainingLabel, shortName, summaryLine, usedFilaments,
} from "./logic.js";
...
export function mount(root, options) {
  ...
  const onMode = typeof settings.onMode === "function" ? settings.onMode : () => {};

  const ui = {
    main: root.getElementById("main"),
    status: root.getElementById("status"),
    stateLabel: root.getElementById("state-label"),
  ...
  const app = {
    ...
    signature: "",
    toastTimer: 0,
    // Window only: what is on screen, the file put aside, the launch sent.
    mode: "hidden",
    dismissedKey: "",
    launchedAt: 0,
    launchedName: "",
  ...
  function render() {
    const model = app.model;
    if (overlay) {
      const mode = overlayMode(app, model, Date.now());
      if (mode !== app.mode) {
        app.mode = mode;
        onMode(mode, model);
      }
      if (mode === "hidden" || mode === "minimised") {
        // Nothing is drawn while the window is away; it is redrawn in full
    ...
    const signature = JSON.stringify([
      ...
      app.armed, app.files, app.mode,
    ]);
    if (signature === app.signature) {
      const live = ui.main.querySelector("#elapsed");
    ...
    else if (model.view === "choice") renderChoice(model);
  ...
  // The window, just after the launch it sent: the print left, here is what
  // it took; the window then closes on its own and Mainsail takes over.
  function renderLaunched(model) {
    const name = model.printingName || model.confirmedName || app.launchedName;
    ui.main.append(hero(
      "Impression lancée",
      shortName(name),
      "Les bobines sont raccordées, la tête chauffe maintenant. Mainsail prend le relais ; "
      + "cette fenêtre se ferme toute seule.",
      "is-ok"));
    ui.main.append(h("section", { class: "pane pane-spools is-wide" },
      h("h3", null, h("span", null, "Les bobines du CFS")),
      spoolGrid(model, null, false, model.confirmedMap)));
  }
  ...
  // ------------------------------------------------------------------- boot
  poll();
  const timer = setInterval(poll, Number(settings.pollMs) || POLL_MS);
  return {
    poll,
    minimise,
    restore,
