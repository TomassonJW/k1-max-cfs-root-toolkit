...
param(
    ...
    [string]$CaptureId = ((Get-Date -Format 'yyyyMMdd-HHmmss') + '-return-t2c-to-t1a-v1'),
...
$ExpectedObserverSha256 = '90813b446b951ff0208e2492274a9bed9d882afd1fd24736a076b949826d89f2'
$WorkspaceRoot = (Resolve-Path (Join-Path $PSScriptRoot '..\..\..')).Path
$RawRoot = Join-Path $WorkspaceRoot 'inventory\raw'
$SessionDirectory = Join-Path $RawRoot $CaptureId
$CapturePath = Join-Path $SessionDirectory 'return-t2c-to-t1a.safe.jsonl'
$AnalysisPath = Join-Path $SessionDirectory 'return-t2c-to-t1a.analysis.json'
...
if ($Action -eq 'Plan') {
    [ordered]@{
        status = 'RETURN_T2C_TO_T1A_PLAN_OK'
        mission = $Mission
...
if ($Action -eq 'Observe' -and (-not $HumanPresent -or -not $ImmediateStopAvailable -or -not $HumanConfirmedT2CIdentity -or -not $HumanConfirmedT1AIdentity)) {
    throw 'Observation réelle refusée : présence, arrêt immédiat et identités T2C/T1A doivent être confirmés.'
}
...
if ($Action -eq 'Observe') {
    ...
    Write-Host 'Après la ligne kind=header, change T2C vers T1A UNE SEULE FOIS depuis l interface stock.'
...
if ($SshExitCode -ne 0 -or $AnalysisExitCode -ne 0) {
    Write-Host "RETURN_T2C_TO_T1A_CLOSED_KO action=$Action capture=$CapturePath"
    exit 1
}
Write-Host "RETURN_T2C_TO_T1A_OK action=$Action capture=$CapturePath"
exit 0
