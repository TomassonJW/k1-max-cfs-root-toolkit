"""Passive observer for one human stock return from T2C to T1A."""
...
MISSION = "G4-K1-CONTROL-RETURN-T2C-TO-T1A-V1"
...
def validate_preflight(item):
    ...
    if item["cfs"]["engaged_routes"] != [START_ROUTE]:
        raise ObserverError("starting_route_not_T2C")
...
def preflight():
    ...
    emit({
        "kind": "preflight",
        "mission": MISSION,
        "status": "RETURN_T2C_TO_T1A_PREFLIGHT_OK",
        "snapshot": item,
...
def observe(duration_s):
    ...
    emit({
        ...
        "operator_action": "stock_ui_change_T2C_to_T1A_once",
    ...
    emit({
        ...
        "hashes_after": after,
        "status": "RETURN_T2C_TO_T1A_OBSERVATION_OK" if unchanged else "RETURN_T2C_TO_T1A_OBSERVATION_KO_CONFIGURATION_DRIFT",
    })
    return unchanged
...
def main(argv=None):
    ...
    try:
        ...
        emit({"kind": "error", "status": "RETURN_T2C_TO_T1A_OBSERVATION_KO", "error": "%s:%s" % (type(exc).__name__, exc)})
        return 1
    return 0
