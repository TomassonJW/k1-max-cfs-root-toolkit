"""Passive observer for one human stock T1A re-engagement from no route."""
...
MISSION = "G4-K1-CONTROL-REENGAGE-T1A-PASSIVE-V1"
...
def snapshot(elapsed_s):
    ...
    return {
        ...
        "calibration": {
            ...
            "low_moves_armed": runtime.get("low_moves_armed"),
            "armed_mesh_profile": runtime.get("armed_mesh_profile"),
        },
        "owner": {
            "phase": owner.get("phase"),
            "watchdog_armed": owner.get("watchdog_armed"),
            "manual_clean_token": owner.get("manual_clean_token"),
        },
        "cfs": {
            "state": box.get("state"),
...
def validate_static(item):
    ...
    if item["calibration"]["low_moves_armed"] not in (0, 0.0):
        raise ObserverError("low_moves_armed")
    if item["calibration"]["armed_mesh_profile"] not in (None, "", "none"):
        raise ObserverError("armed_mesh_profile_not_clear")
    if item["owner"]["phase"] != "idle" or item["owner"]["watchdog_armed"] not in (0, 0.0):
        raise ObserverError("start_owner_not_idle")
    if item["owner"]["manual_clean_token"] not in (0, 0.0):
...
def validate_preflight(item):
    ...
    if item["cfs"]["engaged_routes"] != []:
        raise ObserverError("starting_route_not_empty")
...
def preflight():
    ...
    emit({
        "kind": "preflight",
        "mission": MISSION,
        "status": "REENGAGE_T1A_PASSIVE_PREFLIGHT_OK",
        "snapshot": item,
        "effects": {"gcode": False, "cfs_action": False, "remote_write": False},
    })


def is_stable_terminal_target(item):
    return (
        item["cfs"]["engaged_routes"] == [TARGET_ROUTE]
        and item["cfs"]["active_command"] in (None, "")
        and finite(item["nozzle"].get("target"), "nozzle_target_invalid") == 0.0
        and finite(item["bed"].get("target"), "bed_target_invalid") == 0.0
    )
...
def observe(duration_s):
    ...
    emit({
        ...
        "operator_action": "stock_ui_extrusion_T1A_once",
    ...
    while time.monotonic() - started < duration_s:
        ...
        emit(item)
        count += 1
        stable_target_reads = stable_target_reads + 1 if is_stable_terminal_target(item) else 0
        if stable_target_reads >= 4:
            break
    ...
    emit({
        ...
        "stable_terminal_target_reads": stable_target_reads,
        "configuration_unchanged": unchanged,
        "hashes_after": after,
        "status": "REENGAGE_T1A_PASSIVE_OBSERVATION_OK" if unchanged else "REENGAGE_T1A_PASSIVE_OBSERVATION_KO_CONFIGURATION_DRIFT",
    })
    return unchanged
...
def main(argv=None):
    ...
    try:
        ...
        emit({"kind": "error", "status": "REENGAGE_T1A_PASSIVE_OBSERVATION_KO", "error": "%s:%s" % (type(exc).__name__, exc)})
        return 1
    return 0
