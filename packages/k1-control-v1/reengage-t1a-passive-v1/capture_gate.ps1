...
param(
    ...
    [string]$CaptureId = ((Get-Date -Format 'yyyyMMdd-HHmmss') + '-reengage-t1a-passive-v1'),
    ...
    [switch]$HumanConfirmedT1AIdentity
)

$ErrorActionPreference = 'Stop'
$Mission = 'G4-K1-CONTROL-REENGAGE-T1A-PASSIVE-V1'
$ExpectedObserverSha256 = '66bb382765566901bf52a4f287b0a7c5c3d810470470ea67ca71e61b3df8c66d'
$WorkspaceRoot = (Resolve-Path (Join-Path $PSScriptRoot '..\..\..')).Path
$RawRoot = Join-Path $WorkspaceRoot 'inventory\raw'
$SessionDirectory = Join-Path $RawRoot $CaptureId
$CapturePath = Join-Path $SessionDirectory 'reengage-t1a-passive.safe.jsonl'
$AnalysisPath = Join-Path $SessionDirectory 'reengage-t1a-passive.analysis.json'
...
if ($Action -eq 'Plan') {
    [ordered]@{
        status = 'REENGAGE_T1A_PASSIVE_PLAN_OK'
        mission = $Mission
...
if ($Action -eq 'Observe' -and (-not $HumanPresent -or -not $ImmediateStopAvailable -or -not $HumanConfirmedT1AIdentity)) {
    throw 'Observation réelle refusée : présence, arrêt immédiat et identité T1A doivent être confirmés.'
}
...
$Metadata = [ordered]@{
    ...
    human_confirmed_t1a_identity = [bool]$HumanConfirmedT1AIdentity
}
...
if ($Action -eq 'Observe') {
    Write-Host 'NE DECLENCHE PAS ENCORE L EXTRUSION.'
    Write-Host 'Après la ligne kind=header, lance Extrusion T1A UNE SEULE FOIS depuis l interface stock.'
...
if ($SshExitCode -ne 0 -or $AnalysisExitCode -ne 0) {
    Write-Host "REENGAGE_T1A_PASSIVE_CLOSED_KO action=$Action capture=$CapturePath"
    exit 1
}
Write-Host "REENGAGE_T1A_PASSIVE_OK action=$Action capture=$CapturePath"
exit 0
