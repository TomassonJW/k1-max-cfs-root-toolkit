"""Read-only observer for the one-shot human T1A route restoration gate."""
...
QUERY_PATH = (
    ...
    "&gcode_macro+KCTRL_START_OWNER_STATE=phase,watchdog_armed,abort_latched"
)
...
def validate_preflight(server, snapshot, hashes):
    ...
    if snapshot["cfs"]["engaged_routes"] != []:
        raise ObserverError("route_already_engaged")
    if finite(snapshot["nozzle"]["target_c"], "nozzle_target_invalid") != 0.0:
        raise ObserverError("nozzle_target_not_zero")
    if finite(snapshot["bed"]["target_c"], "bed_target_invalid") != 0.0:
        raise ObserverError("bed_target_not_zero")
    ...
    if snapshot["start_owner"].get("phase") != "idle":
        raise ObserverError("start_owner_not_idle")
    if snapshot["start_owner"].get("watchdog_armed") not in (0, 0.0):
        raise ObserverError("start_owner_watchdog_armed")
...
def collect_preflight():
    ...
    snapshot = safe_snapshot(0.0, fetch_json(QUERY_PATH))
    validate_preflight(server, snapshot, hashes)
    return {
        "kind": "preflight",
        "schema": 1,
        "mission": MISSION,
        "status": "START_SEQUENCE_T1A_ROUTE_V1_PREFLIGHT_OK",
        "hashes": hashes,
        "snapshot": snapshot,
...
def run(duration_s):
    ...
    print(json.dumps({
        "kind": "footer",
        "status": "T1A_ROUTE_OBSERVATION_OK" if configuration_unchanged else "T1A_ROUTE_OBSERVATION_KO_CONFIGURATION_DRIFT",
...
def main(argv=None):
    ...
    if arguments[0] == "preflight":
        try:
            ...
            print(json.dumps({"status": "T1A_ROUTE_PREFLIGHT_KO", "error": "%s:%s" % (type(exc).__name__, exc)}, sort_keys=True))
            return 1
        return 0
    try:
        ...
        return 2
    if not 30.0 <= duration_s <= 300.0:
        print(json.dumps({"status": "INVALID_DURATION"}, sort_keys=True))
        return 2
    try:
        unchanged = run(duration_s)
    except Exception as exc:
        print(json.dumps({"status": "T1A_ROUTE_OBSERVATION_KO", "error": "%s:%s" % (type(exc).__name__, exc)}, sort_keys=True))
        return 1
