"""Rewind confirmation correction: defer the end, resolve the live slot, prove the cut.
...
class KctrlEnd:
    ...
    def _ready(self):
        ...
        for name in ('communication_set_box_mode',
                     ...
                     'communication_retrude_process', 'update_filament_pos'):
            if not callable(getattr(action, name, None)):
                raise self.printer.config_error('kctrl_end: missing primitive ' + name)
        save = getattr(action, 'box_save', None)
        if save is None or not all(hasattr(save, name) for name in ('last_cmd', 'last_cmd_tmp')):
            raise self.printer.config_error('kctrl_end: missing stock withdrawal bookkeeping')
    ...
    def _separated_rewind(self, run):
        ...
        action = self.printer.lookup_object('box').box_action
        # Preserve the exact stock command until this attempt has succeeded.
        # Tn_data.filament is maintained by the host, not a physical sensor.
        saved_cmd = action.box_save.last_cmd
        ...
        # All three acknowledgements have succeeded. The exact stock wrapper
        # then calls update_filament_pos(route, None) and clears last_cmd.
        # Elementary communication_retrude_process does neither: waiting for
        # its cached route to clear caused the real 2026-09-23 false timeout.
        # Require a stable empty head first; never commit a failed withdrawal.
        ...
        while True:
            ...
            if committed and route is not None:
                raise EndRefused('route_reappeared_after_rewind')
            if self.proof.failed or not self.proof.complete:
                raise EndRefused('cutter_release_lost')
            clear = not self._head()
            ...
            if clear:
                ...
                if now - clear_since >= .5:
                    if committed:
                        return
                    if action.box_save.last_cmd != saved_cmd:
                        raise EndRefused('stock_command_changed_during_rewind')
                    # Same bookkeeping and order as the installed stock
                    # BoxAction.box_retrude_material success branch. This
                    # primitive only updates Tn_data, with no serial/motion.
                    action.box_save.last_cmd_tmp = saved_cmd
                    action.update_filament_pos(run['route'], None)
                    action.box_save.last_cmd = None
                    self._check(run)
                    if self._route() is not None or self._head():
                        raise EndRefused('rewind_bookkeeping_not_confirmed')
                    committed = True
                    clear_since = now
                    logging.info('kctrl_end: acknowledged withdrawal committed route=%s', run['route'])
            else:
                clear_since = None
            if now >= deadline:
    ...
    def get_status(self, eventtime=None):
        return {'enabled': self.enabled, 'phase': self.phase, 'reason': self.reason,
                ...
                'physical_validation': False, 'revision': 'end-rewind-confirm-v3',
