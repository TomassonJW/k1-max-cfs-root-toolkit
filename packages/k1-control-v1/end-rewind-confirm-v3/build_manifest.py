...
def build():
    old = json.loads((ROOT/'packages/k1-control-v1/end-overlap-v2/manifest.json').read_text(encoding='utf-8'))
    ...
    return {'schema':1,'name':'cfs-end-rewind-confirm-v3','enabled':True,'physical_validation':False,
            'baseline_capture':'20260923-cfs-rewind-confirmation/initial','before':before,'files':files,
            'installer_sha256':hashlib.sha256(installer.read_bytes()).hexdigest(),
            'backup':'/usr/data/k1-control-v1/backups/cfs-end-rewind-confirm-v3',
