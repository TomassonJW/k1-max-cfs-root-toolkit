"""ADR-069 candidate: defer the end, resolve the live slot, prove the cut.

Disabled by default; no handler is replaced and no output hook is installed
until enabled at klippy:ready. Not deployed. See end-after-refill-candidate.md.
...
class KctrlEnd:
    ...
    def _ready(self):
        ...
        for name in ('heaters', 'extruder', 'heater_bed', 'virtual_sdcard',
                     'pause_resume', 'box', 'filament_switch_sensor filament_sensor_2'):
        ...
        try:
            ...
            with self.gcode.get_mutex():
                ...
                if not head:
                    ...
                    self._script(run, 'BOX_RETRUDE_MATERIAL_WITH_TNN TNN=' + run['route'])
