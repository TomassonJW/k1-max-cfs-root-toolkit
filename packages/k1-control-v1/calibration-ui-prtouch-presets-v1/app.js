...
function syncMatrixAlgorithm() {
  const matrixField = byId("matrix-size");
  const algorithmField = byId("algorithm");
  const matrix = Number(matrixField.value);
  const lagrangeOption = algorithmField.querySelector('option[value="lagrange"]');
  const requiresBicubic = matrix > 6;
  lagrangeOption.disabled = requiresBicubic;
  if (requiresBicubic) {
    algorithmField.value = "bicubic";
  } else if (algorithmField.value === "bicubic" && matrix < 4) {
    matrixField.value = "5";
  }
}
