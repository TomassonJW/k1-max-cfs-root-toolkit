...
BACKUP = Path('/usr/data/k1-control-v1/backups/cfs-end-overlap-v2')
...
def state():
    return api('/printer/objects/query?webhooks=state&print_stats=state&pause_resume&extruder=target,temperature&heater_bed=target,temperature&toolhead=homed_axes,position&kctrl_end&box&motion_report=live_velocity,live_extruder_velocity&bed_mesh=profile_name,probed_matrix&gcode_move=homing_origin&filament_switch_sensor+filament_sensor_2=filament_detected')['status']
...
def cold(s, before_restart=False):
    ...
    allowed = ('', 'xy') if before_restart else ('',)
    ...
    for key in ('live_velocity', 'live_extruder_velocity'):
        assert s['motion_report'][key] == 0, 'motion_active'
...
def owner(s, revision):
    assert s['kctrl_end']['enabled'] is True, 'owner_disabled'
    assert s['kctrl_end']['phase'] == 'idle', 'owner_not_idle'
...
def preflight_state(revision):
    ...
    owner(first, revision)
    time.sleep(.5)
    second = state()
    cold(second, before_restart=True)
    owner(second, revision)
...
def run(request):
    ...
    assert manifest['name'] == 'cfs-end-overlap-v2'
    ...
    if request['mode'] == 'preflight':
        hashes(before_pins)
        before = preflight_state('separated-end-v1')
    ...
    if request['mode'] == 'validate':
        hashes(after_pins)
        after = wait_ready('end-overlap-v2')
    ...
    if request['mode'] == 'rollback':
        hashes(after_pins)
        before = json.loads((BACKUP/'state.before.json').read_text())
        preflight_state('end-overlap-v2')
        for f in files:
            assert digest(BACKUP/(f['name']+'.before')) == before_pins[f['destination']]
        oldpid = stop()
        for f in files:
