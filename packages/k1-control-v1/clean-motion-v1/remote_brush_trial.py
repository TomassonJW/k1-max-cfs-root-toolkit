...
TRIALS = {
    "e1": {
        "before": [203.0, 273.0, 32.0],
        "after": [203.0, 273.0, 32.0],
        "script": (
            "G90\n"
            "G1 X99 Y303 Z32 F1200\n"
            "G1 Z5 F300\n"
            "G1 Y305 F300\n"
            "G1 X66 F300\n"
            "G1 X99 F300\n"
        ...
        "description": "air_trace_3mm_above_primary_brush_and_stop_3mm_before_secondary_brush",
        "brush_contact": "none",
        "minimum_commanded_z_mm": 5.0,
    },
    "e2": {
        "before": [203.0, 273.0, 32.0],
        "after": [81.0, 280.0, 32.0],
        "script": (
            "G90\n"
            "G1 X99 Y303 Z32 F1200\n"
            "G1 Z12 F600\n"
        ...
        "description": "one_slow_primary_brush_contact_from_x99_to_x66_at_y305_z2",
        "brush_contact": "primary_bed_brush_once",
        "minimum_commanded_z_mm": 2.0,
    },
    "e3": {
        "before": [203.0, 273.0, 32.0],
        "after": [203.0, 273.0, 32.0],
        ...
        "description": "secondary_brush_out_and_back_at_y304_5_with_0_5mm_stop_margin_at_z32",
        "brush_contact": "secondary_purge_bin_brush_out_and_back",
        "minimum_commanded_z_mm": 32.0,
    },
    "e4": {
        "before": [203.0, 273.0, 32.0],
        "after": [203.0, 273.0, 32.0],
        ...
        "description": "one_observation_cycle_on_secondary_brush_square_x203_206_y304_305",
        "brush_contact": "secondary_purge_bin_brush_square_cycle_once",
        "minimum_commanded_z_mm": 32.0,
    },
}
...
def send_reviewed_script(script):
    allowed = set(item["script"] for item in TRIALS.values())
    if script not in allowed:
        raise GateError("gcode_not_reviewed")
    request = {"id": 5804, "method": "gcode/script", "params": {"script": script}}
...
def effects(attempted, trial):
    return {
        ...
        "cfs_action": False,
        "homing": False,
        "xy_z_motion": bool(attempted),
        "brush_contact": trial.get("brush_contact") if attempted else "none",
...
def run_preflight(trial_id):
    trial = TRIALS[trial_id]
    before = capture_snapshot()
    validate_position(before, trial["before"], "unexpected_preflight_position")
    return {
        "schema": 1,
        "mission": MISSION,
        "action": "%s_preflight" % trial_id,
        "status": "%s_PREFLIGHT_OK" % trial_id.upper(),
        "trial": trial,
        "before": before,
...
def run_trial(trial_id):
    trial = TRIALS[trial_id]
    before = capture_snapshot()
    validate_position(before, trial["before"], "unexpected_start_position")
    attempted = trial["script"].split("\n")
    try:
        send_reviewed_script(trial["script"])
        after = capture_snapshot()
        validate_position(after, trial["after"], "unexpected_end_position")
        require_equal(after["hashes"], before["hashes"], "configuration_changed")
        return {
            "schema": 1,
            "mission": MISSION,
            "action": trial_id,
            "status": "%s_TECHNICAL_OK_AWAITING_HUMAN_VERDICT" % trial_id.upper(),
            "trial": trial,
            "before": before,
            "after": after,
            "effects": effects(attempted, trial),
        }
        ...
        return {
            ...
            "mission": MISSION,
            "action": trial_id,
            "status": "%s_KO_NO_RETRY_STATE_SAFE" % trial_id.upper() if recovery_error is None else "%s_KO_STATE_UNCERTAIN" % trial_id.upper(),
            "trial": trial,
            "before": before,
...
def main(argv=None):
    ...
    if len(arguments) != 2 or arguments[0] not in ("preflight", "run") or arguments[1] not in TRIALS:
        print(json.dumps({"mission": MISSION, "status": "INVALID_ACTION"}, sort_keys=True))
        return 2
    mode, trial_id = arguments
    try:
        result = run_preflight(trial_id) if mode == "preflight" else run_trial(trial_id)
    except Exception as exc:
        result = {
            "schema": 1,
            "mission": MISSION,
            "action": "%s_%s" % (trial_id, mode),
            "status": "%s_PREFLIGHT_KO" % trial_id.upper(),
            "error": "%s:%s" % (type(exc).__name__, exc),
            "effects": effects([], TRIALS[trial_id]),
        }
    print(json.dumps(result, sort_keys=True, separators=(",", ":")))
    print("CLEAN_MOTION_V1_%s" % result["status"])
    return 0 if result["status"] in (
        "%s_PREFLIGHT_OK" % trial_id.upper(),
        "%s_TECHNICAL_OK_AWAITING_HUMAN_VERDICT" % trial_id.upper(),
    ) else 1
