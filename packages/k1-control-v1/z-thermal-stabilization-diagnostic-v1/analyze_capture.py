...
def analyze_records(records: list[dict]) -> dict:
    ...
    soak_standby = [
        ...
        if item["print"].get("state") == "standby"
    ...
    effects = [item for item in records if item.get("kind") == "effect"]
    started_soaks = [
        item for item in effects
        if item.get("effect") == "bed_thermal_soak_started_once"
        and item.get("requested_soak_s") == 200.0
    ]
    completed_soaks = [
        item for item in effects
        if item.get("effect") == "bed_thermal_soak_completed_once"
        and item.get("requested_soak_s") == 200.0
    ]
    soak_effect_order = [
        item.get("effect")
        for item in effects
        if item.get("effect") in ("bed_thermal_soak_started_once", "bed_thermal_soak_completed_once")
    ]
    soak_observed = 0.0
    if len(soak_standby) >= 2:
        soak_observed = soak_standby[-1]["elapsed_s"] - soak_standby[0]["elapsed_s"]
    if len(completed_soaks) == 1:
        soak_observed = max(soak_observed, float(completed_soaks[0].get("observed_soak_s", 0.0)))
    phases = []
    ...
    soak_nozzle_targets = sorted({item["nozzle"].get("target") for item in soak_standby})
    checks = [
        footer.get("status") == "Z_THERMAL_STABILIZATION_DIAGNOSTIC_AUTOMATION_OK",
        len(started_soaks) == 1,
        len(completed_soaks) == 1,
        soak_effect_order == ["bed_thermal_soak_started_once", "bed_thermal_soak_completed_once"],
        soak_observed >= 195.0,
