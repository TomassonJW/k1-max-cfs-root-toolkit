...
const RUNTIME_OBJECT = "gcode_macro KCTRL_STATE";
const NUMBER = "-?(?:\\d+(?:\\.\\d+)?|\\.\\d+)";
...
const EXACT_COMMANDS = new Set([
  ...
  "KCTRL_MESH_CLEAR_ACTIVE",
]);
const COMMAND_PATTERNS = [
  new RegExp(`^KCTRL_Z_SESSION_START SEED=${NUMBER} PLATE=${POSITIVE_INTEGER} TEMP_BAND=${NON_NEGATIVE_INTEGER} PROBE_REV=${POSITIVE_INTEGER} NOZZLE_ID=${POSITIVE_INTEGER} CONFIG_ID=${POSITIVE_INTEGER}$`),
  new RegExp(`^KCTRL_Z_COMMIT ACCEPTED_AT=${POSITIVE_INTEGER}$`),
  /^KCTRL_Z_ADJUST DELTA=(?:-0\.1|-0\.05|-0\.01|-0\.005|0\.005|0\.01|0\.05|0\.1)$/,
  new RegExp(`^KCTRL_CALIBRATION_PREHEAT BED_TEMP=${NUMBER} NOZZLE_TEMP=${NUMBER} SOAK_SECONDS=${NON_NEGATIVE_INTEGER}$`),
  new RegExp(`^KCTRL_MESH_CALIBRATE X_COUNT=${POSITIVE_INTEGER} Y_COUNT=${POSITIVE_INTEGER} ALGORITHM=(?:lagrange|bicubic)$`),
  new RegExp(`^KCTRL_MESH_COMMIT PLATE=${POSITIVE_INTEGER} TEMP_BAND=${NON_NEGATIVE_INTEGER} PROBE_REV=${POSITIVE_INTEGER} X_COUNT=${POSITIVE_INTEGER} Y_COUNT=${POSITIVE_INTEGER}$`),
];
